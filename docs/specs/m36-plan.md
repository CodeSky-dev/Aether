...
3. **新建 `app/realms/[id]/audit/page.tsx`**
   - 渲染 NavShell（带 `currentRealmId` 和 `currentRealmName`）
   - 调用 listAuditLogs，展示列表或空态
   - 支持 actorType / action 过滤与分页加载更多（P1-7 修复）
4. **更新 `components/nav-shell.tsx`**
   - 接受 `currentRealmId` 和 `currentRealmName` 两个 prop
   - 当 `currentRealmId` 存在时，在 Sidebar 中额外显示"Thread 列表"和"审计记录"两个链接
   - 高亮判定使用精确路径匹配，避免子路由误高亮（P1-8 修复）
