...
drizzle ORM: auditLog table WHERE realm_id = ? ORDER BY created_at DESC LIMIT ?
  ↓
AuditRow { id, actor_type, actor_id, action, doc_ref?, entity_id?, payload_hash, created_at }
  ↓
AuditRowItem × N
```
...
- [ ] 访问 `/realms/[id]/audit` 显示审计记录列表（或空态）
- [ ] NavShell 侧边栏在 `/realms/[id]` 下显示"审计记录"链接
- [ ] 点击"审计记录"跳转到 `/realms/[id]/audit`
- [ ] 无 TypeScript 类型错误
- [ ] ESLint 无报错
- [ ] 现有 test 仍然通过
