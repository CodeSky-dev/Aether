...
export default function RealmCard({ realm }: RealmCardProps) {
  return (
    <Link
      ...
      className="block rounded-lg border border-border bg-neutral-1 p-4 transition hover:border-neutral-3 hover:shadow-sm"
    >
      <div className="mb-1 flex items-center gap-2">
        <h3 className="text-copy-14 font-medium text-neutral-9">{realm.name}</h3>
        <span className="text-copy-11 text-neutral-4">· {realm.slug}</span>
      </div>
      <p className="text-copy-12 text-neutral-5">
        ID: <span className="font-mono text-neutral-6">{realm.id}</span>
      </p>
      <p className="mt-2 text-copy-11 text-neutral-4">
