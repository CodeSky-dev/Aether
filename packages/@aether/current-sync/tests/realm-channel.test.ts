...
describe('RealmChannelRegistry', () => {
  it('隔离不同 Realm 的同名频道，并要求显式注册后访问', () => {
    ...
    expect(registry.get('realm-a', 'current')).toBe(first)
    expect(() => registry.get('realm-b', 'missing')).toThrow('not registered')
  ...
  it('释放频道后不复用旧 Y.Doc', () => {
    const registry = new RealmChannelRegistry()
    const first = registry.open('realm-a', 'current')
    registry.destroy('realm-a', 'current')

    expect(registry.size).toBe(0)
    expect(() => registry.get('realm-a', 'current')).toThrow('not registered')
