  ...
  const callback: RemoteCallback = async (sql, params) => {
    ...
    if (sql.includes('insert into "threads"')) {
      const newRow: MockThread = {
        ...
        code_anchor: params[5] as Record<string, unknown>, manifestation_url: params[6] as string | null,
        dialogue_ref: params[7] as string | null, resolution_contract: params[8] as Record<string, unknown> | null,
