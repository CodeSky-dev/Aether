...
describe('Realm Isolation Guards', () => {
  ...
  const render = (condition: ReturnType<typeof realmGuard>) =>
  ...
  describe('realmGuard', () => {
    it('should generate correct SQL for threads table', () => {
      ...
      expect(render(guard)).toContain('realm_id')
    })
    ...
    it('should handle realms table specially by comparing id', () => {
      ...
      expect(render(guard)).toContain('id')
    })

    it('should reject non-realm tables', () => {
      // 创建一个模拟的不支持 realm 隔离的表
      const fakeTable = { name: 'fake_table' } as unknown as typeof threads
    ...
    it('should reject tables missing realm_id column', () => {
      // 创建一个没有 realm_id 的表
      const tableWithoutRealmId = { name: 'projects' } as unknown as typeof projects
  ...
  describe('realmScope', () => {
    it('should return only guard when no additional conditions provided', () => {
      ...
      expect(render(scope)).toContain('realm_id')
    })
  ...
  describe('Different realm IDs produce different guards', () => {
    it('should generate different guards for different realm IDs', () => {
      ...
      expect(dialect.sqlToQuery(guard1).params).not.toEqual(
        dialect.sqlToQuery(guard2).params,
      )
    })
  })
})
